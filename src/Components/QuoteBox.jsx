...
const QuoteBox = () => {
    ...
    return (
        <div className='phraseBox' style={ { color: colors[randomColor] } }>
            ...
            <h1 className='phrase'><i class="fa-solid fa-quote-left" style={ { color: colors[randomColor] } }></i> {quotes[index].quote}</h1>
            <h1 className='author'>{quotes[index].author}</h1>
            
            <img src="" alt="" />
            <button className='button' onClick={changeQuote} style={ { color: colors[randomColor] } }><i class="fa-solid fa-shuffle"></i></button>
        </div>
    );
};
